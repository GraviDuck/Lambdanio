...
#include <qt/lambdanioaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Lambdanio address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new LambdanioAddressEntryValidator(parent));
    widget->setCheckValidator(new LambdanioAddressCheckValidator(parent));
}
...
bool parseLambdanioURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no lambdanio: URI
    if(!uri.isValid() || uri.scheme() != QString("lambdanio"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!LambdanioUnits::parse(LambdanioUnit::LDO, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseLambdanioURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseLambdanioURI(uriInstance, out);
}

QString formatLambdanioURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("lambdanio:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(LambdanioUnits::format(LambdanioUnit::LDO, info.amount, false, LambdanioUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openLambdanioConf()
{
    ...
    /* Open lambdanio.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Lambdanio.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Lambdanio (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Lambdanio (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Lambdanio*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "lambdanio.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("lambdanio-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a lambdanio.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Lambdanio\n";
        else
            optionFile << strprintf("Name=Lambdanio (%s)\n", ChainTypeToString(chain));
