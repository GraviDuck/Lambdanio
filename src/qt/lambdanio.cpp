...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<LambdanioUnit>("LambdanioUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. lambdanio_de.qm (shortcut "de" needs to be defined in lambdanio.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. lambdanio_de_DE.qm (shortcut "de_DE" needs to be defined in lambdanio.qrc)
...
static const char* qt_argv = "lambdanio-qt";

LambdanioApplication::LambdanioApplication()
...
void LambdanioApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the LambdanioApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", LambdanioGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void LambdanioApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool LambdanioApplication::createOptionsModel(bool resetSettings)
{
...
void LambdanioApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new LambdanioGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &LambdanioGUI::quitRequested, this, &LambdanioApplication::requestShutdown);
...
void LambdanioApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void LambdanioApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool LambdanioApplication::baseInitialize()
{
    return node().baseInitialize();
}

void LambdanioApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &LambdanioApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &LambdanioApplication::handleRunawayException);
    connect(this, &LambdanioApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &LambdanioApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void LambdanioApplication::parameterSetup()
{
...
void LambdanioApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void LambdanioApplication::requestInitialize()
{
...
void LambdanioApplication::requestShutdown()
{
...
void LambdanioApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // lambdanio: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &LambdanioGUI::handlePaymentRequest);
        connect(window, &LambdanioGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void LambdanioApplication::handleRunawayException(const QString &message)
{
...
void LambdanioApplication::handleNonFatalException(const QString& message)
{
...
WId LambdanioApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool LambdanioApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", LambdanioGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(lambdanio);
    Q_INIT_RESOURCE(lambdanio_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(LAMBDANIO_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see lambdanio-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see lambdanio-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse lambdanio.conf, determine network, switch to network specific
    ...
    // lambdanio: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
