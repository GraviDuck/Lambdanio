...
/** Call node.updateRwSetting() with Lambdanio 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Lambdanio 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/lambdanio/lambdanio/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because lambdanio 22.x will not try to read these.
...
//! Convert enabled/size values to lambdanio -prune setting.
...
//! Get pruning enabled value to show in GUI from lambdanio -prune setting.
...
//! Get pruning size value to show in GUI from lambdanio -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayLambdanioUnit")) {
        settings.setValue("DisplayLambdanioUnit", QVariant::fromValue(LambdanioUnit::LDO));
    }
    QVariant unit = settings.value("DisplayLambdanioUnit");
    if (unit.canConvert<LambdanioUnit>()) {
        m_display_lambdanio_unit = unit.value<LambdanioUnit>();
    } else {
        m_display_lambdanio_unit = LambdanioUnit::LDO;
        settings.setValue("DisplayLambdanioUnit", QVariant::fromValue(m_display_lambdanio_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if lambdanio.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_lambdanio_unit);
...
{
    if (new_unit.isNull() || new_unit.value<LambdanioUnit>() == m_display_lambdanio_unit) return;
    m_display_lambdanio_unit = new_unit.value<LambdanioUnit>();
    QSettings settings;
    settings.setValue("DisplayLambdanioUnit", QVariant::fromValue(m_display_lambdanio_unit));
    Q_EMIT displayUnitChanged(m_display_lambdanio_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/lambdanio/lambdanio/pull/8273
    ...
    // (https://github.com/lambdanio-core/gui/issues/567).
