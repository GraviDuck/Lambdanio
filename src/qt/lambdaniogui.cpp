...
#include <lambdanio-build-config.h> // IWYU pragma: keep

#include <qt/lambdaniogui.h>
 ...
 *
 * Ref: https://github.com/lambdanio/lambdanio/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string LambdanioGUI::DEFAULT_UIPLATFORM =
...
LambdanioGUI::LambdanioGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &LambdanioGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &LambdanioGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &LambdanioGUI::showModalOverlay);
...
void LambdanioGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Lambdanio address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and lambdanio: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &LambdanioGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &LambdanioGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &LambdanioGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Lambdanio addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Lambdanio addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Lambdanio Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Lambdanio Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a lambdanio: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Lambdanio command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &LambdanioGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &LambdanioGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &LambdanioGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &LambdanioGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &LambdanioGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &LambdanioGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &LambdanioGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &LambdanioGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &LambdanioGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &LambdanioGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &LambdanioGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &LambdanioGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &LambdanioGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &LambdanioGUI::showDebugWindow);
}

void LambdanioGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void LambdanioGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &LambdanioGUI::setCurrentWalletBySelectorIndex);
...
void LambdanioGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &LambdanioGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &LambdanioGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &LambdanioGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &LambdanioGUI::showProgress);
...
#ifdef ENABLE_WALLET
void LambdanioGUI::enableHistoryAction(bool privacy)
{
...
void LambdanioGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &LambdanioGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &LambdanioGUI::removeWallet);
...
WalletController* LambdanioGUI::getWalletController()
{
    return m_wallet_controller;
}

void LambdanioGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &LambdanioGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &LambdanioGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &LambdanioGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &LambdanioGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &LambdanioGUI::incomingTransaction);
    connect(this, &LambdanioGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void LambdanioGUI::removeWallet(WalletModel* walletModel)
{
...
void LambdanioGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void LambdanioGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void LambdanioGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void LambdanioGUI::setWalletActionsEnabled(bool enabled)
{
...
void LambdanioGUI::createTrayIcon()
{
...
void LambdanioGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &LambdanioGUI::toggleHidden);
...
void LambdanioGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void LambdanioGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void LambdanioGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void LambdanioGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void LambdanioGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void LambdanioGUI::openClicked()
{
...
void LambdanioGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void LambdanioGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void LambdanioGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void LambdanioGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void LambdanioGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void LambdanioGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void LambdanioGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void LambdanioGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Lambdanio network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void LambdanioGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void LambdanioGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void LambdanioGUI::updateHeadersSyncProgressLabel()
{
...
void LambdanioGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void LambdanioGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &LambdanioGUI::quitRequested);
...
void LambdanioGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void LambdanioGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &LambdanioGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void LambdanioGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void LambdanioGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &LambdanioGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &LambdanioGUI::show);
                e->ignore();
            }
        }
    }
...
void LambdanioGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void LambdanioGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void LambdanioGUI::incomingTransaction(const QString& date, LambdanioUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(LambdanioUnits::formatWithUnit(unit, amount, true));
...
void LambdanioGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void LambdanioGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool LambdanioGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool LambdanioGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void LambdanioGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void LambdanioGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void LambdanioGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void LambdanioGUI::updateProxyIcon()
{
    std::string ip_port;
...
void LambdanioGUI::updateWindowTitle()
{
...
void LambdanioGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void LambdanioGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void LambdanioGUI::detectShutdown()
{
...
void LambdanioGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void LambdanioGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(LambdanioGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = LambdanioGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void LambdanioGUI::subscribeToCoreSignals()
{
...
void LambdanioGUI::unsubscribeFromCoreSignals()
{
...
bool LambdanioGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<LambdanioUnit> units = LambdanioUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const LambdanioUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, LambdanioUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const LambdanioUnit u : LambdanioUnits::availableUnits()) {
        menu->addAction(LambdanioUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(LambdanioUnit newUnits)
{
    setText(LambdanioUnits::longName(newUnits));
}
