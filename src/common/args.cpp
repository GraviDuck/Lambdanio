...
const char * const LAMBDANIO_CONF_FILENAME = "lambdanio.conf";
const char * const LAMBDANIO_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //lambdanio-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", LAMBDANIO_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Lambdanio
    //   new: C:\Users\Username\AppData\Local\Lambdanio
    // macOS: ~/Library/Application Support/Lambdanio
    // Unix-like: ~/.lambdanio
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Lambdanio";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Lambdanio";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Lambdanio";
#else
    // Unix-like
    return pathRet / ".lambdanio";
#endif
#endif
}
