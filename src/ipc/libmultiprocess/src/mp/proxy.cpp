...
thread_local ThreadContext g_thread_context; // NOLINT(lambdanio-nontrivial-threadlocal)
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking lambdanio code so they have to run
    ...
    // functions run, so client objects are fully disconnected before lambdanio
    // code in the destructors are run. This way if the lambdanio code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In lambdanio core, running these callbacks in the right order is
