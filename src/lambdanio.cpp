// Copyright (c) 2025-present The Lambdanio Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries lambdanio-node, lambdanio-gui.
  -M, --monolithic       Run monolithic binaries lambdaniod, lambdanio-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'lambdanio-qt [ARGS]' or 'lambdanio-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'lambdaniod [ARGS]' or 'lambdanio-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'lambdanio-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'lambdanio-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'lambdanio-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_lambdanio [ARGS]'.
  chainstate [ARGS] Run lambdanio kernel chainstate util, equivalent to running 'lambdanio-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_lambdanio [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_lambdanio-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "lambdanio-gui" : "lambdanio-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "lambdanio-node" : "lambdaniod");
        } else if (cmd.command == "rpc") {
            args.emplace_back("lambdanio-cli");
            // Since "lambdanio rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("lambdanio-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("lambdanio-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_lambdanio");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("lambdanio-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_lambdanio");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_lambdanio-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("lambdanio-util");
        } else {
...
//! Execute the specified lambdaniod, lambdanio-qt or other command line in `args`
...
//!             be a relative path to a lambdaniod, lambdanio-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/lambdanio ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/lambdanio/lambdanio/pull/31375#discussion_r1861814807)
