...
namespace init {
namespace {
const char* EXE_NAME = "lambdanio-gui";

class LambdanioGuiInit : public interfaces::Init
{
public:
    LambdanioGuiInit(const char* arg0) : m_ipc(interfaces::MakeIpc(EXE_NAME, arg0, *this))
    {
        InitContext(m_node);
        m_node.init = this;
    }
    ...
    // lambdanio-gui accepts -ipcbind option even though it does not use it
    // directly. It just returns true here to accept the option because
    // lambdanio-node accepts the option, and lambdanio-gui accepts all lambdanio-node
...
{
    return std::make_unique<init::LambdanioGuiInit>(argc > 0 ? argv[0] : "");
}
} // namespace interfaces
