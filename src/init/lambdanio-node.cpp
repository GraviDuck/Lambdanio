...
namespace init {
namespace {
const char* EXE_NAME = "lambdanio-node";

class LambdanioNodeInit : public interfaces::Init
{
public:
    LambdanioNodeInit(node::NodeContext& node, const char* arg0)
        : m_node(node),
...
{
    auto init = std::make_unique<init::LambdanioNodeInit>(node, argc > 0 ? argv[0] : "");
    // Check if lambdanio-node is being invoked as an IPC server. If so, then
