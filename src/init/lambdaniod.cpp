...
namespace init {
namespace {
const char* EXE_NAME = "lambdaniod";

class LambdaniodInit : public interfaces::Init
{
public:
    LambdaniodInit(NodeContext& node) : m_node(node)
    {
        InitContext(m_node);
        m_node.init = this;
    }
...
{
    return std::make_unique<init::LambdaniodInit>(node);
}
} // namespace interfaces
