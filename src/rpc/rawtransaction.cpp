                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in lambdanio-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in lambdanio-common, so we query them here and push the
...
static RPCMethod decodepsbt()
{
    return RPCMethod{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed Lambdanio transaction.",
                {
...
static RPCMethod combinepsbt()
{
    return RPCMethod{
        "combinepsbt",
        "Combine multiple partially signed Lambdanio transactions into one transaction.\n"
