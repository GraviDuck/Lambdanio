...
//! Specify whether parameter should be parsed by lambdanio-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make lambdanio-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `lambdanio-cli` attempt to parse the
 ...
 * `lambdanio-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make lambdanio-cli treat them as positional parameters when `-named` is used.
 * This prevents `lambdanio-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so lambdanio-cli
 ...
 *
 * For example, the command `lambdanio-cli -named createwallet "my=wallet"`,
