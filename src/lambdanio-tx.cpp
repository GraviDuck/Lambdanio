...
static void SetupLambdanioTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " lambdanio-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The lambdanio-tx tool is used for creating and modifying lambdanio transactions.\n\n"
                "lambdanio-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded lambdanio transaction, or with \"-create [commands]\" to create a hex-encoded lambdanio transaction.\n"
                "\n"
                "Usage: lambdanio-tx [options] <hex-tx> [commands]\n"
                "or:    lambdanio-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded lambdanio transaction
