...
be used in Lambdanio Core with the `-asmap` option.
...
to binary before being used with Lambdanio Core.
...
the `lambdanio-cli getnodeaddresses` command.
...
```
lambdanio-cli getnodeaddresses 0 > addrs.json
```
