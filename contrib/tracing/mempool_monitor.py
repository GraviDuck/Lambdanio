...
""" Example logging Lambdanio Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into lambdaniod with pid {pid}")
    lambdaniod_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    lambdaniod_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    lambdaniod_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    lambdaniod_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    lambdaniod_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[lambdaniod_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of lambdaniod>")
        exit(1)
