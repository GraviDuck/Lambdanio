# Bootstrappable Lambdanio Core Builds

This directory contains the files necessary to perform bootstrappable Lambdanio
Core builds.
...
To build Lambdanio Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Lambdanio Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/lambdanio-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/lambdanio-core/guix.sigs).
