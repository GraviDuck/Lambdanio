#!/usr/bin/env bash
# Copyright (c) 2019-present The Lambdanio Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/lambdanio-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Lambdanio Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Lambdanio Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example lambdanio.conf file. if contrib/devtools/gen-lambdanio-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/lambdanio.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/lambdanio-macos-app.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
