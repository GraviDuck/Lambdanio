...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing lambdanio-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='lambdanio-cli', help='lambdanio-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Lambdanio address to which the faucet should send')
...
parser.add_argument('lambdanio_cli_args', nargs='*', help='Arguments to pass on to lambdanio-cli (default: -signet)')

args = parser.parse_args()

if args.lambdanio_cli_args == []:
    args.lambdanio_cli_args = ['-signet']


def lambdanio_cli(rpc_command_and_params):
    argv = [args.cmd] + args.lambdanio_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = lambdanio_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = lambdanio_cli(['getnewaddress', 'faucet', 'bech32'])
