...
"""Script for verifying Lambdanio Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from lambdaniocore.org and lambdanio.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/lambdanio-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://lambdaniocore.org"
HOST2 = "https://lambdanio.org"
VERSIONPREFIX = "lambdanio-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Lambdanio community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"lambdanio_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by lambdaniocore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the lambdanio release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes lambdanio.org lags behind lambdaniocore.org.)')
    )
