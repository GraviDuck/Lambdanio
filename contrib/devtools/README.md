...
gen-lambdanio-conf.sh
===================

Generates a lambdanio.conf file in `share/examples/` by parsing the output from `lambdaniod --help`. This script is run during the
release process to include a lambdanio.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-lambdanio-conf.sh
```
