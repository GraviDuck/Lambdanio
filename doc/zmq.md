...
The Lambdanio Core daemon can be configured to act as a trusted "border
router", implementing the lambdanio wire protocol and relay, making
...
The ZeroMQ feature in Lambdanio Core requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ lambdaniod -zmqpubhashtx=tcp://127.0.0.1:29332 \
               -zmqpubhashtx=tcp://192.168.1.2:29332 \
               -zmqpubhashblock="tcp://[::1]:29333" \
               -zmqpubrawtx=unix:/tmp/lambdaniod.tx.raw \
...
From the perspective of lambdaniod, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into lambdaniod directly. Furthermore, no information is
...
using. Lambdaniod appends an up-counting sequence number to each
