...
Below are some notes on how to build Lambdanio Core for Windows.

The options known to work for building Lambdanio Core on Windows are:
...
Note that for WSL the Lambdanio Core source path MUST be somewhere in the default mount file system, for
example /usr/src/lambdanio, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\lambdanio`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/lambdanio
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/lambdanio --strip
```
