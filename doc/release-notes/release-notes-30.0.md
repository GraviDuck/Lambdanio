...
Lambdanio Core version v30.0 is now available from:

  <https://lambdaniocore.org/bin/lambdanio-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/Lambdanio-Qt` (on macOS)
...
Lambdanio Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Lambdanio
...
New `lambdanio` command
---------------------

- A new `lambdanio` command line tool has been added to make features more discoverable
  and convenient to use. The `lambdanio` tool just calls other executables and does not
  implement any functionality on its own. Specifically `lambdanio node` is a synonym for
  `lambdaniod`, `lambdanio gui` is a synonym for `lambdanio-qt`, and `lambdanio rpc` is a synonym
  for `lambdanio-cli -named`. Other commands and options can be listed with `lambdanio help`.
  The new `lambdanio` command is an alternative to calling other commands directly, but it
...
- The new `lambdanio` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `lambdanio -m node -ipcbind=unix` it will
  ...
  (`lambdanio-node` instead of `lambdaniod`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/lambdanio/lambdanio/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/lambdanio/lambdanio/blob/master/doc/multiprocess.md)),
...
- The `test_lambdanio` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `lambdanio` command
  as `lambdanio test`. The `libexec/` directory also contains new `lambdanio-node` and
  `lambdanio-gui` binaries which support IPC features and are called through the
  `lambdanio` tool. In source builds only, `test_lambdanio-qt`, `bench_lambdanio`, and
  `lambdanio-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `lambdanio` command. See `lambdanio help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Lambdanio Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Lambdanio Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the lambdanio-wallet tool,
...
[Transifex](https://explore.transifex.com/lambdanio/lambdanio/).
