Lambdanio Core version 28.0 is now available from:

  <https://lambdaniocore.org/bin/lambdanio-core-28.0/>
...
Running Lambdanio Core binaries on macOS requires self signing.
```
cd /path/to/lambdanio-28.0/bin
xattr -d com.apple.quarantine lambdanio-cli lambdanio-qt lambdanio-tx lambdanio-util lambdanio-wallet lambdaniod test_lambdanio
codesign -s - lambdanio-cli lambdanio-qt lambdanio-tx lambdanio-util lambdanio-wallet lambdaniod test_lambdanio
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Lambdanio
...
Support for Testnet4 as specified in [BIP94](https://github.com/lambdanio/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Lambdanio`
to `C:\Users\Username\AppData\Local\Lambdanio`. Lambdanio Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/lambdanio/lambdanio/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
liblambdanioconsensus Removal
---------------------------

The liblambdanio-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Lambdanio Core was listening for P2P connections, either using
  ...
  of binding to `0.0.0.0:9333` and `127.0.0.1:8334` has not been changed.
...
- Lambdanio Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/lambdanio/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Lambdanio Core with the configuration
...
are now required to compile Lambdanio Core. (#29091, #30263)

- The minimum required glibc to run Lambdanio Core is now
...
- A new `-testdatadir` option has been added to `test_lambdanio` to allow specifying the
...
Previous releases of Lambdanio Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
