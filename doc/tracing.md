# User-space, Statically Defined Tracing (USDT) for Lambdanio Core

Lambdanio Core includes statically defined tracepoints to allow for more
...
**Note**: Lambdanio Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Lambdanio Core
...
Multiple tools can list the available tracepoints in a `lambdaniod` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Lambdanio Core, use `info probes` in `gdb`:

```
$ gdb ./build/bin/lambdaniod
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/lambdaniod
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/lambdaniod
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/lambdaniod
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Lambdanio Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/lambdaniod | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./build/bin/lambdaniod -v
