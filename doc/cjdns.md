# CJDNS support in Lambdanio Core

It is possible to run Lambdanio Core over CJDNS, an encrypted IPv6 network that
...
done outside of Lambdanio Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Lambdanio network and individual nodes.
...
## Run Lambdanio Core with CJDNS

Once you are connected to the CJDNS network, the following Lambdanio Core
...
When enabled, this option tells Lambdanio Core that it is running in an
...
IPv6 local network. This helps Lambdanio Core perform better address management:
...
CJDNS support was added to Lambdanio Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `lambdanio-cli -addrinfo` to see the
...
## CJDNS-related information in Lambdanio Core

There are several ways to see your CJDNS address in Lambdanio Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `lambdanio-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `lambdanio-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `lambdanio-cli help getnodeaddresses` for details.
