...
# The block reward of coinbaseoutput.nValue (50) LDO/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 LDO/block.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"lambdanio-cli -h\" for help or \"lambdanio-cli listwallets\" to see "
...
class TestLambdanioCli(LambdanioTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `lambdanio-cli getblockchaininfo`")
        ...
        # Prevent lambdanio-cli from using existing rpcport in conf
        ...
        self.log.info("Test -getinfo and lambdanio-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and lambdanio-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (LDO/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 LDO) to the default wallet.
            ...
            # Test lambdanio-cli -generate with -rpcwallet in multiwallet mode.
        ...
        if not self.is_ipc_compiled():
            ...
            self.log.info("Test lambdanio-cli -ipcconnect triggers error if not built with IPC support")
            args = [self.binary_paths.lambdaniocli, "-ipcconnect=unix", "-getinfo"]
            result = subprocess.run(args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            assert_equal(result.stdout, "error: lambdanio-cli was not built with IPC support\n")
            assert_equal(result.stderr, None)
            assert_equal(result.returncode, 1)


if __name__ == '__main__':
    TestLambdanioCli(__file__).main()
