...
without the parent process' LambdanioTestFramework also failing.
"""
...
class FeatureFrameworkStartupFailures(LambdanioTestFramework):
    ...
    def _verify_startup_failure(self, test, internal_args, expected_exception):
        ...
        try:
            ...
            sys.exit("Unexpected child process timeout!\n"
                     ...
                     "meaning dangling lambdaniod processes are to be expected.\n" +
    ...
    def run_test(self):
        ...
        self.log.info("Verifying inability to connect to lambdaniod's RPC interface due to wrong port results in one exception containing at least one OSError.")
        self._verify_startup_failure(
            TestWrongRpcPortStartupFailure, [f"--internal_node_start_duration={node_start_duration}"],
            r"AssertionError: \[node 0\] Unable to connect to lambdaniod after \d+s \(ignored errors: {[^}]*'OSError \w+'?: \d+[^}]*}, latest: '[\w ]+'/\w+\([^)]+\)\)"
        )
        ...
        self._verify_startup_failure(
            ...
            r"AssertionError: \[node 0\] lambdaniod should have exited within \d+s with an error \(cmd:"
        )

        self.log.info("Verifying startup failure due to invalid arg results in only one exception.")
        self._verify_startup_failure(
            TestInitErrorStartupFailure, [],
            r"FailedToStartError: \[node 0\] lambdaniod exited with status 1 during initialization\. Error: Error parsing command line arguments: Invalid parameter -nonexistentarg"
        )
...
class InternalDurationTestMixin(InternalTestMixin):
    ...
    def get_reasonable_rpc_timeout(self):
        ...
        # Divide by timeout_factor to counter multiplication in LambdanioTestFramework.
        return max(3, 2 * self.options.node_start_duration) / self.options.timeout_factor

class TestWrongRpcPortStartupFailure(InternalDurationTestMixin, LambdanioTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestMissingInitErrorTimeout(InternalDurationTestMixin, LambdanioTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestInitErrorStartupFailure(InternalTestMixin, LambdanioTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestStartStopStartupFailure(InternalTestMixin, LambdanioTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestSuccess(InternalTestMixin, LambdanioTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
