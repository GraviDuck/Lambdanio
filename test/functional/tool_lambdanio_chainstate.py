...
"""Test lambdanio-chainstate tool functionality

Test basic block processing via lambdanio-chainstate tool, including detecting
duplicates and malformed input.

Test that lambdanio-chainstate can load a datadir initialized with an assumeutxo
...
class LambdanioChainstateTest(LambdanioTestFramework):
    def skip_test_if_missing_module(self):
        self.skip_if_no_lambdanio_chainstate()
    ...
    def basic_test(self):
        ...
        self.log.info(f"Test lambdanio-chainstate {self.get_binaries().chainstate_argv()} with datadir: {datadir}")
    ...
    def assumeutxo_test(self, dump_output_path):
        ...
        datadir = n1.chain_path
        n1.stop_node()
        self.log.info(f"Test lambdanio-chainstate {self.get_binaries().chainstate_argv()} with an assumeutxo datadir: {datadir}")
...
if __name__ == "__main__":
    LambdanioChainstateTest(__file__).main()
