...
`test/functional/test_framework/test_framework.py:LambdanioTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_lambdanio_cli.py',
    'feature_bind_extra.py',
    'mempool_resurrect.py',
    'wallet_txn_doublespend.py --mineblock',
    'tool_lambdanio_chainstate.py',
    'tool_wallet.py',
    'tool_utils.py',
...
def main():
    ...
    enable_lambdaniod = config["components"].getboolean("ENABLE_LAMBDANIOD")

    if not enable_lambdaniod:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if lambdaniod is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "lambdaniod"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a lambdaniod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Lambdanio functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/lambdanio/lambdanio/pull/27561.
...
class RPCCoverage():
    ...
    commands per `lambdanio-cli help` (`rpc_interface.txt`).
