...
class P2PPrivateBroadcast(LambdanioTestFramework):
    ...
    def run_test(self):
        ...
        for addr in ADDRMAN_ADDRESSES:
            res = tx_originator.addpeeraddress(address=addr, port=0 if addr.endswith(".i2p") else 9333, tried=False)
            if not res["success"]:
        ...
        # Stop the SOCKS5 proxy server to avoid it being upset by the lambdanio
        ...
        self.restart_node(0, extra_args=[
            ...
            # Lambdanio Core to start, hoping/assuming that the location of the Tor proxy
