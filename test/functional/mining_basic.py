...
class MiningTest(LambdanioTestFramework):
    ...
    def test_blockmintxfee_parameter(self):
        ...
        for blockmintxfee_sat_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 0, 5, 10, 50, 100, 500, 1000, 2500, 5000, 21000, 333333, 2500000):
            blockmintxfee_ldo_kvb = blockmintxfee_sat_kvb / Decimal(COIN)
            if blockmintxfee_sat_kvb == DEFAULT_BLOCK_MIN_TX_FEE:
                ...
                blockmintxfee_parameter = f"-blockmintxfee={blockmintxfee_ldo_kvb:.8f}"
            ...
            assert_equal(node.getmininginfo()['blockmintxfee'], blockmintxfee_ldo_kvb)

            # submit one tx with exactly the blockmintxfee rate, and one slightly below
            tx_with_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_ldo_kvb, confirmed_only=True)
            assert_equal(tx_with_min_feerate["fee"], get_fee(tx_with_min_feerate["tx"].get_vsize(), blockmintxfee_ldo_kvb))
            if blockmintxfee_sat_kvb >= 10:
                lowerfee_ldo_kvb = blockmintxfee_ldo_kvb - Decimal(10)/COIN  # 0.01 sat/vbyte lower
                assert_greater_than(blockmintxfee_ldo_kvb, lowerfee_ldo_kvb)
                assert_greater_than_or_equal(lowerfee_ldo_kvb, 0)
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=lowerfee_ldo_kvb, confirmed_only=True)
                assert_equal(tx_below_min_feerate["fee"], get_fee(tx_below_min_feerate["tx"].get_vsize(), lowerfee_ldo_kvb))
            else:  # go below zero fee by using modified fees
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_ldo_kvb, confirmed_only=True)
