...
# Copyright 2016-present The Lambdanio Core developers
...
class ToolUtils(LambdanioTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_lambdanio_tx()
        self.skip_if_no_lambdanio_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "lambdanio-util-test.json") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./lambdanio-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./lambdanio-tx":
