...
class AvoidReuseTest(LambdanioTestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        '''
        Test the same as test_sending_from_reused_address_fails, except send the 10 LDO with
        the avoid_reuse flag set to false. This means the 10 LDO send should succeed,
        ...
        # listunspent should show 1 single, unused 10 ldo output
        assert_unspent(self.nodes[1], total_count=1, total_sum=10, reused_supported=True, reused_count=0)
        # getbalances should show no used, 10 ldo trusted
        ...
        # listunspent should show 1 single, unused 5 ldo output
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_supported=True, reused_count=0)
        # getbalances should show no used, 5 ldo trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        self.nodes[0].sendtoaddress(fundaddr, 10)
        self.generate(self.nodes[0], 1)

        # listunspent should show 2 total outputs (5, 10 ldo), one unused (5), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=15, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 5 ldo trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 5})

        self.nodes[1].sendtoaddress(address=retaddr, amount=10, avoid_reuse=False)

        # listunspent should show 1 total outputs (5 ldo), unused
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_count=0)
        # getbalances should show no used, 5 ldo trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 ldo left (for both cases)
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 LDO to A.
        [1] spends 5 LDO from A. (leaving roughly 5 LDO useable)
        [0] sends 10 LDO to A again.
        [1] tries to spend 10 LDO (fails; dirty).
        [1] tries to spend 4 LDO (succeeds; change address sufficient)
        '''
    ...
    def test_full_destination_group_is_preferred(self):
        '''
        Test the case where [1] only has 101 outputs of 1 LDO in the same reused
        address and tries to send a small payment of 0.5 LDO. The wallet
        should use 100 outputs from the reused address as inputs and not a
        single 1 LDO input, in order to join several outputs from the reused
        address.
        '''
        ...
        # Send 101 outputs of 1 LDO to the same, reused address in the wallet
        for _ in range(101):
            self.nodes[0].sendtoaddress(new_addr, 1)
    ...
    def test_all_destination_groups_are_used(self):
        '''
        Test the case where [1] only has 202 outputs of 1 LDO in the same reused
        address and tries to send a payment of 200.5 LDO. The wallet
        ...
        # Send 202 outputs of 1 LDO to the same, reused address in the wallet
        for _ in range(202):
            self.nodes[0].sendtoaddress(new_addr, 1)
