...
TMPDIR_PREFIX = "lambdanio_func_test_"
...
class LambdanioTestMetaClass(type):
    """Metaclass for LambdanioTestFramework.

    Ensures that any attempt to register a subclass of `LambdanioTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'LambdanioTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("LambdanioTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("LambdanioTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class LambdanioTestFramework(metaclass=LambdanioTestMetaClass):
    """Base class for a lambdanio test script.

    Individual lambdanio test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave lambdaniods and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use lambdanio-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("LambdanioRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a lambdaniod test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple lambdaniod test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the lambdaniod
    ...
    def _start_logging(self):
        ...
        # Format logs the same as lambdaniod's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in lambdanio.conf
    ...
    def skip_if_no_lambdaniod_tracepoints(self):
        """Skip the running test if lambdaniod has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("lambdaniod has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_lambdaniod_zmq(self):
        """Skip the running test if lambdaniod has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("lambdaniod has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if lambdanio-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("lambdanio-wallet has not been compiled")

    def skip_if_no_lambdanio_tx(self):
        """Skip the running test if lambdanio-tx has not been compiled."""
        if not self.is_lambdanio_tx_compiled():
            raise SkipTest("lambdanio-tx has not been compiled")

    def skip_if_no_lambdanio_util(self):
        """Skip the running test if lambdanio-util has not been compiled."""
        if not self.is_lambdanio_util_compiled():
            raise SkipTest("lambdanio-util has not been compiled")

    def skip_if_no_lambdanio_chainstate(self):
        """Skip the running test if lambdanio-chainstate has not been compiled."""
        if not self.is_lambdanio_chainstate_compiled():
            raise SkipTest("lambdanio-chainstate has not been compiled")

    def skip_if_no_lambdanio_bench(self):
        """Skip the running test if bench_lambdanio has not been compiled."""
        if not self.is_bench_compiled():
            raise SkipTest("bench_lambdanio has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if lambdanio-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("lambdanio-cli has not been compiled.")
    ...
    def is_bench_compiled(self):
        """Checks whether bench_lambdanio was compiled."""
        return self.config["components"].getboolean("BUILD_BENCH")

    def is_cli_compiled(self):
        """Checks whether lambdanio-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether lambdanio-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_lambdanio_tx_compiled(self):
        """Checks whether lambdanio-tx was compiled."""
        return self.config["components"].getboolean("BUILD_LAMBDANIO_TX")

    def is_lambdanio_util_compiled(self):
        """Checks whether lambdanio-util was compiled."""
        return self.config["components"].getboolean("ENABLE_LAMBDANIO_UTIL")

    def is_lambdanio_chainstate_compiled(self):
        """Checks whether lambdanio-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_LAMBDANIO_CHAINSTATE")
