#!/usr/bin/env python3
# Copyright (c) 2014-present The Lambdanio Core developers
...
def assert_fee_amount(fee, tx_size, feerate_LDO_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_LDO_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s LDO too low! (Should be %s LDO)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_LDO_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s LDO too high! (Should be %s LDO)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting LDO values"""
...
class Binaries:
    """Helper class to provide information about lambdanio binaries
    ...
    def __init__(self, paths, bin_dir, *, use_valgrind=False):
        ...
        self.valgrind_cmd = [
            ...
            "--trace-children=yes",  # Needed for 'lambdanio' wrapper
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke lambdaniod"
        return self._argv("node", self.paths.lambdaniod, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke lambdanio-cli"
        # Add -nonamed because "lambdanio rpc" enables -named by default, but lambdanio-cli doesn't
        return self._argv("rpc", self.paths.lambdaniocli) + ["-nonamed"]

    def bench_argv(self):
        "Return argv array that should be used to invoke bench_lambdanio"
        return self._argv("bench", self.paths.lambdanio_bench)

    def tx_argv(self):
        "Return argv array that should be used to invoke lambdanio-tx"
        return self._argv("tx", self.paths.lambdaniotx)

    def util_argv(self):
        "Return argv array that should be used to invoke lambdanio-util"
        return self._argv("util", self.paths.lambdanioutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke lambdanio-wallet"
        return self._argv("wallet", self.paths.lambdaniowallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke lambdanio-chainstate"
        return self._argv("chainstate", self.paths.lambdaniochainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command.

        It either uses the lambdanio wrapper executable (if LAMBDANIO_CMD is set or
        need_ipc is True), or the direct binary path (lambdaniod, etc). When
        ...
        (previous releases). Also, valgrind will only apply to the lambdanio
        wrapper executable directly, not to the commands that `lambdanio` calls.
        """
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.lambdanio_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the lambdanio
            # wrapper binary and append -m so it calls multiprocess binaries.
            lambdanio_cmd = self.paths.lambdanio_cmd or [self.paths.lambdanio_bin]
            return self.valgrind_cmd + lambdanio_cmd + (["-m"] if need_ipc else []) + [command]
        else:
...
def get_binary_paths(config):
    ...
    binaries = {
        "lambdanio": "LAMBDANIO_BIN",
        "lambdaniod": "LAMBDANIOD",
        "bench_lambdanio": "LAMBDANIO_BENCH",
        "lambdanio-cli": "LAMBDANIOCLI",
        "lambdanio-util": "LAMBDANIOUTIL",
        "lambdanio-tx": "LAMBDANIOTX",
        "lambdanio-chainstate": "LAMBDANIOCHAINSTATE",
        "lambdanio-wallet": "LAMBDANIOWALLET",
    }
    # Set paths to lambdanio core binaries allowing overrides with environment
    # variables.
    ...
    # LAMBDANIO_CMD environment variable can be specified to invoke lambdanio
    # wrapper binary instead of other executables.
    paths.lambdanio_cmd = shlex.split(os.getenv("LAMBDANIO_CMD", "")) or None
    return paths
...
def get_fee(tx_size, feerate_ldo_kvb):
    """Calculate the fee in LDO given a feerate is LDO/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_ldo_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  LDO
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `LambdanioTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/lambdanio/lambdanio/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "lambdanio.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Lambdanio"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Lambdanio"
        else:
            datadir = temp_dir / ".lambdanio"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "lambdanio.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "lambdanio.conf")):
        with open(os.path.join(datadir, "lambdanio.conf"), 'r') as f:
            for line in f:
